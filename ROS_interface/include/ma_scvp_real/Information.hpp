...
public:
	...
	Views_Information(Share_Data* share_data, Voxel_Information* _voxel_information , View_Space* view_space,int iterations)
	{
		...
		num_of_max_thread = share_data->num_of_max_thread;
		//注意视点需要按照id排序来建立映射
		...
		//分配视点的射线生成器
		thread** ray_caster = new thread * [num_of_max_thread];
		//射线初始下标从0开始
		ray_num = 0;
		for (int i = 0; i < view_space->views.size(); i += num_of_max_thread) {
			for (int j = 0; j < num_of_max_thread && i + j < view_space->views.size(); j++) {
				//对该视点分发生成射线的线程
				ray_caster[j] = new thread(ray_cast_thread_process, &ray_num, rays_info, rays_map, views_to_rays_map, rays_to_viwes_map, octo_model, voxel_information, view_space, &color_intrinsics, i + j);
			}
			//等待每个视点射线生成器计算完成
			for (int j = 0; j < num_of_max_thread && i + j < view_space->views.size(); j++) {
				(*ray_caster[j]).join();
		...
		//为每条射线分配一个线程
		now_time = clock();
		thread** rays_process = new thread * [num_of_max_thread];
		for (int i = 0; i < ray_num; i += num_of_max_thread) {
			for (int j = 0; j < num_of_max_thread && i + j < ray_num; j++) {
				rays_process[j] = new thread(ray_information_thread_process, i + j, rays_info, rays_map, occupancy_map, object_weight, octo_model, voxel_information, view_space, method);
			}
			//等待射线计算完成
			for (int j = 0; j < num_of_max_thread && i + j < ray_num; j++) {
				(*rays_process[j]).join();
		...
		//分配视点的信息统计器
		now_time = clock();
		thread** view_gain = new thread * [num_of_max_thread];
		for (int i = 0; i < view_space->views.size(); i += num_of_max_thread) {
			for (int j = 0; j < num_of_max_thread && i + j < view_space->views.size(); j++) {
				//对该视点分发信息统计的线程
				view_gain[j] = new thread(information_gain_thread_process, rays_info, views_to_rays_map, view_space, i + j);
			}
			//等待每个视点信息统计完成
			for (int j = 0; j < num_of_max_thread && i + j < view_space->views.size(); j++) {
				(*view_gain[j]).join();
...
inline vector<cv::Point2f> get_convex_on_image(vector<Eigen::Vector4d>& convex_3d, Eigen::Matrix4d& now_camera_pose_world, rs2_intrinsics& color_intrinsics,int& pixel_interval,double& max_range,double& octomap_resolution) {
	...
	for (int i = 0; i < convex_3d.size(); i++) {
		...
		float point[3] = { float(vertex(0)), float(vertex(1)), float(vertex(2)) };
		float pixel[2];
...
public:
	...
	views_voxels_MF(int _nx, View_Space* _view_space, Views_Information* _views_information, Voxel_Information* _voxel_information, Share_Data* _share_data) {
		...
		//并行遍历每条射线上的体素累加至对应视点
		nz = 0;
		thread** adjacency_list_process = new thread * [views_information->num_of_max_thread];
		for (int i = 0; i < views_information->ray_num; i += views_information->num_of_max_thread) {
			for (int j = 0; j < views_information->num_of_max_thread && i + j < views_information->ray_num; j++) {
				adjacency_list_process[j] = new thread(adjacency_list_MF_thread_process, i + j, &nz, nx, nx + ny, voxel_id_map, bipartite_list, view_space, views_information, voxel_information, share_data);
			}
			for (int j = 0; j < views_information->num_of_max_thread && i + j < views_information->ray_num; j++) {
				(*adjacency_list_process[j]).join();
			}
			//释放内存
			for (int j = 0; j < views_information->num_of_max_thread && i + j < views_information->ray_num; j++) {
				adjacency_list_process[j]->~thread();
			}
		}
...
public:
	...
	views_voxels_GMC(int _nx, View_Space* _view_space, Views_Information* _views_information, Voxel_Information* _voxel_information, Share_Data* _share_data) {
		...
		for (int i = 0; i < views_information->ray_num; i += views_information->num_of_max_thread) {
			for (int j = 0; j < views_information->num_of_max_thread && i + j < views_information->ray_num; j++) {
				adjacency_list_process[j] = new thread(adjacency_list_GMC_thread_process, i + j, &ny, voxel_id_map, bipartite_list, view_space, views_information, voxel_information, share_data);
			}
			for (int j = 0; j < views_information->num_of_max_thread && i + j < views_information->ray_num; j++) {
				(*adjacency_list_process[j]).join();
			}
