...
void RosInterface::run()
{
    ...
    if(share_data->gt_mode){
        ...
        *share_data->cloud_ground_truth = *share_data->cloud_now;
        ...
        int num_of_path = get_trajectory_xyz(points, now_view_xyz, next_view_xyz, nbv_plan->now_view_space->object_center_world, nbv_plan->now_view_space->predicted_size, share_data->move_dis_pre_point, share_data->camera_to_object_dis);
        if (num_of_path == -1) {
        ...
        for (int i = 0; i < nbv_plan->now_view_space->views.size() - 1; i++) {
            cout<< "view_space_pose.size():" << view_space_pose.size()<<endl;
            // otherwise move robot
            Eigen::Vector3d now_view_xyz(nbv_plan->now_view_space->views[i].init_pos(0), nbv_plan->now_view_space->views[i].init_pos(1), nbv_plan->now_view_space->views[i].init_pos(2));
            Eigen::Vector3d next_view_xyz(nbv_plan->now_view_space->views[i+1].init_pos(0), nbv_plan->now_view_space->views[i+1].init_pos(1), nbv_plan->now_view_space->views[i+1].init_pos(2));
            vector<Eigen::Vector3d> points;
            int num_of_path = get_trajectory_xyz(points, now_view_xyz, next_view_xyz, nbv_plan->now_view_space->object_center_world, nbv_plan->now_view_space->predicted_size, share_data->move_dis_pre_point, share_data->camera_to_object_dis);
            if (num_of_path == -1) {
                cout << "no path. throw" << endl;
            ...
            if (visitWaypoints(waypoints_msg)){
                ...
                if (visitWaypoints(waypoints_msg)){
                    ...
                    View temp_view(nbv_plan->now_view_space->views[i+1]);
                    Eigen::Vector3d now_object_center_world = nbv_plan->now_view_space->object_center_world;
                    //now_object_center_world(2) = share_data->min_z_table;
                    temp_view.get_next_camera_pos(share_data->now_camera_pose_world,  now_object_center_world);
                    Eigen::Matrix4d temp_camera_pose_world = share_data->now_camera_pose_world * temp_view.pose.inverse() * share_data->camera_depth_to_rgb; 
                    Eigen::Matrix3d temp_rotation = temp_camera_pose_world.block<3,3>(0, 0);
                    Eigen::Quaterniond temp_q(temp_rotation);
                    vector<float> temp_waypoint = { float(temp_camera_pose_world(0, 3)), float(temp_camera_pose_world(1, 3)), float(temp_camera_pose_world(2, 3)), float(temp_q.x()), float(temp_q.y()), float(temp_q.z()), float(temp_q.w()) };
                    ...
                    if (visitPose(target_orginal_pose)){
                        num_of_movable_views++;
                        vector<double> current_joint = getJointValues();
                        view_space_pose.push_back(current_joint);
                        ROS_INFO("MoveitClient: Arm moved to target original pose 3");
                        now_camera_pose_world = share_data->now_camera_pose_world * temp_view.pose.inverse();
                    }
                    else{
                        ...
                        if (visitPose(target_table_height_pose)){
                            num_of_movable_views++;
                            vector<double> current_joint = getJointValues();
                            view_space_pose.push_back(current_joint);
    ...
    cout<<"num_of_iterations: "<<nbv_plan->iterations+1<<endl;
    cout<<"num_of_movable_views: "<<num_of_movable_views<<endl;
